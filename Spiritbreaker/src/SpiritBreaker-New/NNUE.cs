...
namespace Spiritbreaker
{
    public static class NNUE
    {
        ...
        static NNUE()
        {
            Stream? s = Assembly.GetExecutingAssembly().GetManifestResourceStream("net.bin");
        ...
        public static int Evaluate(Board board)
        {
            ...
            for (int colour = 0; colour <= 1; colour++)   // 0 = Weiss, 1 = Schwarz
            {
                bool white = colour == 0;
                for (PieceType type = PieceType.Pawn; type <= PieceType.King; type++)
                {
                    int pc = (int)type - 1;                // Pawn..King -> 0..5
                    ulong bb = board.GetPieceBitboard(type, white);
                    while (bb != 0)
                    {
                        int sq = BitboardHelper.ClearAndGetIndexOfLSB(ref bb);   // a1=0..h8=63
