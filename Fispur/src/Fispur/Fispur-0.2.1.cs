...
public class Fispur0_2_1 : IChessBot
{
    ...
    public (Move move, int eval) Think(Board board, Timer timer, int maxDepth = -1)
    {
        ...
        while (timer.MillisecondsElapsedThisTurn < time / 2)
        {
            ...
            Console.WriteLine("info currmove " + FispurEngine.Chess.MoveUtility.GetMoveNameUCI(bestMove.move) + " depth " + depth + " score cp " + eval + " time " + timer.MillisecondsElapsedThisTurn);
            depth++;
        }
